...
$file = __DIR__ . '/../core/class/lampesoirmatinbe.class.php';
...
if (!preg_match('/class\s+lampesoirmatinbeCmd\s+extends\s+cmd/', $source)) {
    $problems[] = 'Classe lampesoirmatinbeCmd absente — impossible de créer un équipement.';
}
 ...
 * fonctionner, et la lampe n'apparaît nulle part ailleurs. */
...
/* ------------------------------------------------------------------ 8 ---
 * La reconnaissance des lampes s'appuie sur les types génériques du coeur : si
 * l'un d'eux disparaît ou change de nom à une mise à jour de Jeedom, le
 * sélecteur cesse de trouver des lampes sans la moindre erreur. */
require_once __DIR__ . '/../core/class/lampesoirmatinbeLamps.class.php';
foreach (lampesoirmatinbeLamps::$_generics as $role => $list) {
    foreach ($list as $generic => $confidence) {
        if (!isset($generics[$generic])) {
            $problems[] = 'Le détecteur cherche un type générique que ce coeur ne connaît pas : '
                . $generic . ' (rôle ' . $role . ') — les lampes qui le portent ne seront plus proposées.';
        }
    }
}
 ...
 *
 * C'est arrivé, sur core/ajax : le sélecteur de lampes tournait indéfiniment.
 *
